...
import {
  BackgroundPatternSectionProps,
  BlockCardSectionProps,
  TestimonialGridSectionProps,
  TitleBlockSectionProps,
  TitleBlocksResponse,
} from "@/types";

export default function TitleBlocksSection({
  data,
}: {
  data: TitleBlockSectionProps;
}) {
        ...
        {titleBlockData.titleBlocks[0]?.conte_blocks &&
          ...
          titleBlockData.titleBlocks[0].conte_blocks.map((data, i) => {
            if (data.__typename == "ComponentBlockBlockCard" || i == 0) {
              return (
                <TabbedShowcase
                  data={data as BlockCardSectionProps}
                  key={data.id}
                />
              );
              ...
              return (
                <TestimonialGrid
                  data={data as TestimonialGridSectionProps}
                  key={data.id}
                />
              );
              ...
              return (
                <BackgroundPatternSection
                  data={data as BackgroundPatternSectionProps}
                  key={data.id}
                />
              );
            } else {
